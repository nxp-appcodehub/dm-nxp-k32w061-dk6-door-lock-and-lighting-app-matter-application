...
CHIP_ERROR AppTask::Init()
{
    ...
    LockMgr().Init();
    LockMgr().SetCallback(LockManagerCallback);
...
void AppTask::LockManagerCallback(LockManager::LockManagerEvent event)
{
    switch (event)
    {
        case LockManager::kLockManagerEvent_LockStarted:
        {
            sLockLED.Set(true);
            LockMgr().SetLockState(LockManager::kLockManagerLockState_Locked);
        } break;
 
        case LockManager::kLockManagerEvent_LockComplete:
        {
            sLockLED.Set(true);
            LockMgr().SetLockState(LockManager::kLockManagerLockState_Locked);
        } break;
 
        case LockManager::kLockManagerEvent_UnlockStarted:
        {
            sLockLED.Set(false);
            LockMgr().SetLockState(LockManager::kLockManagerLockState_Unlocked);
        } break;
 
        case LockManager::kLockManagerEvent_UnlockComplete:
        {
            sLockLED.Set(false);
            LockMgr().SetLockState(LockManager::kLockManagerLockState_Unlocked);
        } break;
    ...
    sAppTask.UpdateDoorLockClusterState();
}
...
void AppTask::UpdateDoorLockClusterState(void)
{
    PlatformMgr().ScheduleWork(UpdateDoorLockClusterStateInternal, 0);
}

void AppTask::UpdateDoorLockClusterStateInternal(intptr_t arg)
{
    LockManager::LockManagerLockState state = LockMgr().GetLockState();

    if (state == LockManager::kLockManagerLockState_Locked)
    {
        Protocols::InteractionModel::Status status = chip::app::Clusters::DoorLock::Attributes::LockState::Set(2, chip::app::Clusters::DoorLock::DlLockState::kLocked);
        if (status != Protocols::InteractionModel::Status::Success)
        {
            ChipLogError(NotSpecified, "ERR: updating lock-state %x", to_underlying(status));
        }
    } else if (state == LockManager::kLockManagerLockState_Unlocked)
    {
        Protocols::InteractionModel::Status status = chip::app::Clusters::DoorLock::Attributes::LockState::Set(2, chip::app::Clusters::DoorLock::DlLockState::kUnlocked);
        if (status != Protocols::InteractionModel::Status::Success)
        {
            ChipLogError(NotSpecified, "ERR: updating lock-state %x", to_underlying(status));
        }
...
void AppTask::UpdateLockDeviceState(void)
{
    PlatformMgr().ScheduleWork(UpdateLockDeviceStateInternal, 0);
}

void AppTask::UpdateLockDeviceStateInternal(intptr_t arg)
{
    Nullable<chip::app::Clusters::DoorLock::DlLockState> state;

    /* get lockstate attribute value */
    chip::app::Clusters::DoorLock::Attributes::LockState::Get(2, state);

    /* set the device state*/
    if (state.Value() == chip::app::Clusters::DoorLock::DlLockState::kLocked)
    {
        sLockLED.Set(true);
        LockMgr().SetLockState(LockManager::kLockManagerLockState_Locked);
    }
    else if (state.Value() == chip::app::Clusters::DoorLock::DlLockState::kUnlocked)
    {
        sLockLED.Set(false);
        LockMgr().SetLockState(LockManager::kLockManagerLockState_Unlocked);
    }
